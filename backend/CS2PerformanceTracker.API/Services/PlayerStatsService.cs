...
public class PlayerStatsService
{
    ...
    public async Task<PlayerStatsResponse?> GetPlayerStats(string input)
    {
        ...
        var mapPerformance = allMatches
            .GroupBy(m => m.MapName)
            .Select(group => new MapPerformanceResponse
            {
                MapName = group.Key,

                MatchesPlayed = group.Count(),

                Wins = group.Count(m => m.Won),

                WinRate = group.Any()
                    ? group.Count(m => m.Won) * 100.0 / group.Count()
                    : 0,

                AverageKd = group.Any()
                    ? group.Average(m => m.KdRatio)
                    : 0
            })
            .OrderByDescending(m => m.MatchesPlayed)
            .ToList();
